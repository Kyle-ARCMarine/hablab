...
- On Mac, download the latest version of the Hablab installer (`.dmg`).<br>
...
- The resolution at which the structure is to be analysed must then be defined, along with the mesh unit that the mesh was created with. This is set to metres by default.
- The structure type must then be defined as a "Sample" or "Structure".
...
- If "Skip to Results" is toggled on, then previously calculated results for a given mesh will be used to accelerate the calculation of results.
- If "Calculate for an accessing diameter", an input table for access classes will be available for user inputs.
